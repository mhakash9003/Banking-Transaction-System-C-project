...
struct account {
    ...
    unsigned long password_hash; // Stores a simple hash of the password
};
...
int verify_password(int account_no, const char *password);
...
void main_menu() {
    ...
    do {
        ...
        printf("==================================================\n");
        printf("1. Create New Account (Set Password)\n");
        printf("2. View Account Details\n");
        printf("3. Deposit Amount\n");
        printf("4. Withdraw Amount (Requires Password)\n");
        printf("5. Transfer Funds (Requires Source Password)\n");
        printf("6. Delete Account (Requires Password)\n");
        ...
        if (scanf("%d", &choice) != 1) {
            ...
            while (getchar() != '\n'); // Clear buffer
            choice = 0; // Force another loop iteration
        }
...
/**
 * @brief Creates a new account, prompts for a password, and writes it to the file.
 */
void create_account() {
    ...
    if (scanf("%d", &new_acc.account_no) != 1) {
    ...
    printf("Enter **Password** (max 49 chars) for this account: ");
    ...
    new_acc.password_hash = simple_hash(password);
    
    printf("Enter Initial Deposit Amount: ");
    if (scanf("%lf", &new_acc.balance) != 1 || new_acc.balance < 0) {
        printf("\nERROR: Invalid or negative deposit amount.\n");
        return;
    }
    ...
    fclose(fp);
    printf("\nAccount %d Created Successfully with a secure password!\n", new_acc.account_no);
}
...
void view_account_details() {
    ...
    if (scanf("%d", &acc_no) != 1) {
        printf("\nERROR: Invalid account number input.\n");
        return;
    }
    ...
    if (get_account_details(acc_no, &acc) == 1) {
        ...
        printf("Name      : %s\n", acc.name);
        printf("Balance   : %.2lf\n", acc.balance);
        // Note: We intentionally DO NOT display the password_hash.
        printf("--------------------------------\n");
    } else {
...
void deposit_amount() {
    ...
    if (scanf("%lf", &amount) != 1 || amount <= 0) {
        printf("\nERROR: Invalid or non-positive deposit amount.\n");
        return;
    }

    // Note: Deposit does not require a password as it only increases funds.
    if (search_and_update(acc_no, amount, 1)) {
        printf("\nDeposit Successful! Account %d updated. New Balance shown in View Details.\n", acc_no);
    } else {
...
/**
 * @brief Withdraws a specified amount from an account. Requires password check.
 */
void withdraw_amount() {
    int acc_no;
    double amount;
    ...
    if (!verify_password(acc_no, password)) {
        printf("\nERROR: Invalid account number or password. Withdrawal failed.\n");
        return;
    }
    // --- END SECURITY CHECK ---
    ...
    if (scanf("%lf", &amount) != 1 || amount <= 0) {
        printf("\nERROR: Invalid or non-positive withdrawal amount.\n");
        return;
    }
    ...
    if (search_and_update(acc_no, amount, 2)) {
        ...
        if (get_account_details(acc_no, &acc) == 1) {
             ...
             printf("\nERROR: Withdrawal failed. Account not found (after password check).\n");
        }
    }
}

/**
 * @brief Transfers funds between two accounts. Requires source account password.
 */
void transfer_funds() {
    ...
    if (scanf("%d", &source_acc_no) != 1) return;
    printf("Enter Destination Account Number: ");
    if (scanf("%d", &dest_acc_no) != 1) return;
    ...
    // 1. Check if source account exists and verify password
    ...
    // --- SECURITY CHECK ---
    printf("Enter Source Account Password: ");
    if (scanf("%49s", password) != 1) return;
    
    if (!verify_password(source_acc_no, password)) {
        printf("\nERROR: Invalid password for Source Account. Transfer aborted.\n");
        return;
    }
    // --- END SECURITY CHECK ---
    ...
    if (scanf("%lf", &amount) != 1 || amount <= 0) {
        printf("\nERROR: Invalid or non-positive transfer amount.\n");
        return;
    }
    ...
    // 4. Process the transfer (Withdraw from source, Deposit to destination)
    // Withdraw from source
    if (search_and_update(source_acc_no, amount, 3) == 0) { // type 3 for Transfer-Out
        printf("\nERROR: Failed to withdraw from Source Account (Unexpected error).\n");
        return;
    }
    ...
    if (search_and_update(dest_acc_no, amount, 4) == 0) { // type 4 for Transfer-In
        ...
        // Logically, a reversal/compensation should be done here in a real system.
        return;
    }
...
/**
 * @brief Deletes an account record. Requires password check.
 */
void delete_account() {
    int acc_no;
    char password[50];
    ...
    if (!verify_password(acc_no, password)) {
        printf("\nERROR: Invalid account number or password. Deletion failed.\n");
        return;
    }
    // --- END SECURITY CHECK ---
...
/**
 * @brief Generates a simple, non-cryptographic hash of a string (djb2).
 * @param str The string (password) to hash.
 * @return An unsigned long hash value.
 */
unsigned long simple_hash(const char *str) {
    // Note: This is a simple hash for demonstration. A real system must use
    // robust cryptographic hashing (like SHA-256) and salting.
...
/**
 * @brief Verifies the provided password against the stored hash for a given account.
...
int verify_password(int account_no, const char *password) {
    ...
    if (get_account_details(account_no, &acc) == 0) {
        return 0; // Account not found
    }

    unsigned long input_hash = simple_hash(password);
    
    // Compare the generated hash with the stored hash
    return (acc.password_hash == input_hash);
}
        ...
        if (acc.account_no == account_no) {
            ...
            if (type == 1 || type == 4) { // Deposit or Transfer-In
                ...
                if (acc.balance >= amount) {
                    ...
                    // Overdraft check failed, write the old record back
                    fwrite(&acc, sizeof(struct account), 1, fp_out);
                    found = 0; // Operation failed due to insufficient funds
                    // Exit the loop immediately to prevent further processing
                    break;
                }
            } else if (type == 5) { // Delete - skip writing the record
                // Found and marked for successful deletion by skipping it
            }
        } else {
